namespace fontaudio
{
  ...
  RenderedIcon IconHelper::getIcon(IconName icon, float size, juce::Colour colour, float scaleFactor)
  {
    ...
    auto identifier = juce::String(icon + "@" + juce::String(scaledSize) + "@" + colour.toString());
    auto hash = identifier.hashCode64();
    auto canvas = juce::ImageCache::getFromHashCode(hash);
    if (canvas.isValid())
      return canvas;
    
    auto FontAudio = getFont((float)scaledSize);
  ...
  {
    int scaledSize = int(size * scaleFactor);
    auto identifier = juce::String(icon + "@" + juce::String(scaledSize) + "@" + colour.toString() + "@" + juce::String(iconRotation) + "@");
    auto hash = identifier.hashCode64();
    auto canvas = juce::ImageCache::getFromHashCode(hash);
    if (canvas.isValid())
      return canvas;
    
    auto renderdIcon = getIcon(icon, size, colour, scaleFactor);
